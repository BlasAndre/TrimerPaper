...
\begin{equation} \label{eq:clhamm}
H = \sum_{j=0}^{2} \omega_{j}(t) n_{j}  
+ \sum_{j \neq k = 0}^{2} g_{jk} \sqrt{n_{j} n_{k}} \cos \left( \phi_{j} - \phi_{k} \right),	
\end{equation}

It proves viable to notice that for a given energy and a given set of parameters generally an infinity number of trajectories exist.
...
E.
Equation (\ref{eq:clhamm}) readily reveals that for given intensities 
...
\begin{figure}[htbp]
	\centering
	\fbox{\includegraphics[height=4cm]{Fig4.pdf}}
