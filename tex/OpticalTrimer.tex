...
We will try our best to bridge the gap between theoretical physics and optics to show how the underlying symmetries of a photonic lattice can shed light into the photonic lattice design process. 
For this, we will use a triangular three-waveguide coupler .
...
\end{array} \right) \left( \begin{array}{c} \mathcal{E}_{0}(z) \\ \mathcal{E}_{1}(z) \\  \mathcal{E}_{2}(z) \end{array} \right). \label{eq:Eq1}
\end{eqnarray}
...
The formal solution for this ordinary linear differential equation,
\begin{eqnarray}
\vert \mathcal{E}(z) \rangle = \hat{U}(z) \vert \mathcal{E}(0) \rangle. \label{eq:Eq3}
\end{eqnarray}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Group theory and propagator}
...
For example, unitary matrices of rank three, just like our mode-coupling matrix $\hat{H}(z)$ in Eq.(\ref{eq:Eq1}), are commonly characterized by the special unitary group $SU(3)$,
\begin{eqnarray}
	\hat{H}(z) &=& \textrm{\textcolor{red}{diagonal elements are missing}} + \\
...
Here, however, we choose a different representation for the group \cite{Ticciati1999}, 
\begin{eqnarray}
...
The literature provides us with actions of elements of the $su(3)$ Lie algebra on elements of the $SU(3)$ group \cite{Nelson1967p857}, the standard Wei-Norman technique for $SU(3)$ \cite{Dattoli1987p1582}, and, here, we will show that the physical restrictions of the optical trimmer allows for further simplification.
...
Any given normalized field vector that solves the Schr\"odinger-like mode coupling equation for a finite dimension system can be written in terms of a $su(3)$ Lie algebra, in other words, the propagator can also be given by the following expression, 
\begin{eqnarray}
	\hat{U}(z) = \prod_{j=1}^{8} e^{i \theta_{j}(z) \hat{X}_{j}}.
\end{eqnarray}
where the algebra elements, $e^{i \theta_{j}(z) \hat{X}_{j}}$, are just the exponential map of the group generators, Eq.(\ref{eq:gens}), the functions $\theta_{j}(z)$ are complex functions ruled by the dynamics provided by the mode-coupling matrix.
...
Substituting the propagated field vector, Eq.(\ref{eq:Eq3}) considering as propagator Eq.(\ref{eq:prop}), into the mode coupling equation, Eq.(\ref{eq:diff}), is a cumbersome but straightforward operation that yields a set of eight coupled differential equations,
\begin{eqnarray}
